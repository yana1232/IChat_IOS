...
class ChatsViewController: MessagesViewController {
    ...
    private func insertNewMessage(message: MMessage) {
        ...
        let shouldScrollToButtom = messagesCollectionView.isAtBottom && isLatestMessage
    ...
    private func sendImage(image: UIImage) {
        StorageService.shared.uploadImageMessage(photo: image, to: chat) { (result) in
            ...
            case .success(let url):
                ...
                FirestoreService.shared.sendMessage(chat: self.chat, messages: message) { (result) in
                    switch result {
                    case .success():
                        self.messagesCollectionView.scrollToLastItem() //scrollToButtom
                    case .failure(_):
...
extension ChatsViewController: InputBarAccessoryViewDelegate {
    func inputBar(_ inputBar: InputBarAccessoryView, didPressSendButtonWith text: String) {
        ...
        FirestoreService.shared.sendMessage(chat: chat, messages: message) { (result) in
            switch result {
            case .success():
                self.messagesCollectionView.scrollToLastItem() //scrollToButtom
            case .failure(let error):
