...
//        }
    ...
    func sendMessage(chat: MChat, messages: MMessage, completion: @escaping (Result<Void, Error>) -> Void) {
        ...
        friendRef.setData(chatForFriend.representation) { (error) in
            ...
            friendMessageRef.addDocument(data: messages.representation) { (error) in
                if let error = error {
                    completion(.failure(error))
                    return
                }
                myMessageRef.addDocument(data: messages.representation) { (error) in
                    if let error = error {
                        completion(.failure(error))
                        return
                    }
